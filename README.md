...
- Coefficients and Insights: Each input variable is assigned a weight, or coefficient, that shows all the strength of its relationship with the purchase decision. For instance, to like the most of like high coefficient for "previous site visits" suggests that retargeting existing fans is more effective than reaching new ones.
- Model Evaluation: Marketers use a "Confusion Matrix" to see how many purchases were correctly predicted versus how many were missed. This is to feedback loop will always allows data scientists to refine the ad campaign for better accuracy and higher return on investment.
